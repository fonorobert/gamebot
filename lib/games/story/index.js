...
export default function (config, { saveState, onFinish }, { maxIterations = 3 } = {}) {
  ...
  return {
    ...
    skipUser () {
      state.playCount += 1
      this.nextUser()
      sendMessage(config, `You snooze you lose! Next is *${this.getCurrentUserName()}*. ${this.calculateRemainingTurns() === 4 ? this.getHurryMessage() : '' } Game progress: ${this.calculateProgress()}%.`)
    },
    ...
    processMessage (message) {
      ...
      if (state.iterationCount == state.maxIterations) {
        ...
        sendMessage(config, `Cool. Next is *${this.getCurrentUserName()}*! ${this.calculateRemainingTurns() === 4 ? this.getHurryMessage() : '' } Game progress: ${this.calculateProgress()}%.`)
      }
    },
    ...
    getHurryMessage () {
      return `Start leading to a conclusion, there are only 3 more people left after you!`
    },
