  ...
  return {
    ...
    processMessage (message) {
      ...
      if (state.iterationCount == state.maxIterations) {
        ...
        const hurry = `Start leading to a conclusion, after you there are only 3 more people left!`
        sendMessage(config, `Cool. Next is *${this.getCurrentUserName()}*! ${this.calculateRemainingTurns() === 4 ? hurry : '' } Game progress: ${this.calculateProgress()}.`)
      }
    },
    ...
    calculateProgress () {
      const { playCount, iterationCount } = state
      const maximumPlayCount = iterationCount * users.length
