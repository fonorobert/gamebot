...
export default function (config, { saveState, onFinish }, { maxIterations = 3 } = {}) {
  ...
  return {
    ...
    start () {
      ...
      const pretext = `Contribute to a story with 3 words. *${this.getCurrentUserName()}* starts`
      const title = 'Rules, tips and tricks'
      const text = [
    ...
    processTitle (message) {
      ...
      say('Good. Anyone else? You can also say _THE END_ if you think there are enough titles.')
      this.setTitleTimeout()
    },
